...
type AuthSourceDefaultSettings struct {
	...
	ForceEmailOnOIDCAccountCreation bool
}
...
func (s *SettingService) GetPublicSettings(ctx context.Context) (*PublicSettings, error) {
	keys := []string{
		...
		SettingKeyForceEmailOnOIDCAccountCreation,
		SettingKeyAccountCreationEmailSuffixWhitelist,
	...
	accountCreationEmailSuffixWhitelist := ParseAccountCreationEmailSuffixWhitelist(
		settings[SettingKeyAccountCreationEmailSuffixWhitelist],
	)
	...
	return &PublicSettings{
		...
		ForceEmailOnOIDCAccountCreation:     settings[SettingKeyForceEmailOnOIDCAccountCreation] == "true",
		AccountCreationEmailSuffixWhitelist: accountCreationEmailSuffixWhitelist,
		...
		SiteSubtitle:                        s.getStringOrDefault(settings, SettingKeySiteSubtitle, "Internal Gateway"),
...
func (s *SettingService) buildSystemSettingsUpdates(ctx context.Context, settings *SystemSettings) (map[string]string, error) {
	normalizedWhitelist, err := NormalizeAccountCreationEmailSuffixWhitelist(settings.AccountCreationEmailSuffixWhitelist)
	if err != nil {
		return nil, infraerrors.BadRequest("INVALID_ACCOUNT_CREATION_EMAIL_SUFFIX_WHITELIST", err.Error())
	}
	if normalizedWhitelist == nil {
		normalizedWhitelist = []string{}
	}
	settings.AccountCreationEmailSuffixWhitelist = normalizedWhitelist

	updates := make(map[string]string)

	// 认证与账号创建设置
	updates[SettingKeyEmailVerifyEnabled] = strconv.FormatBool(settings.EmailVerifyEnabled)
	accountCreationEmailSuffixWhitelistJSON, err := json.Marshal(settings.AccountCreationEmailSuffixWhitelist)
	if err != nil {
		return nil, fmt.Errorf("marshal account creation email suffix whitelist: %w", err)
	}
	updates[SettingKeyAccountCreationEmailSuffixWhitelist] = string(accountCreationEmailSuffixWhitelistJSON)
...
func (s *SettingService) buildAuthSourceDefaultUpdates(ctx context.Context, settings *AuthSourceDefaultSettings) (map[string]string, error) {
	...
	updates := make(map[string]string, 8)
	writeProviderDefaultGrantUpdates(updates, emailAuthSourceDefaultKeys, settings.Email)
	writeProviderDefaultGrantUpdates(updates, oidcAuthSourceDefaultKeys, settings.OIDC)
	updates[SettingKeyForceEmailOnOIDCAccountCreation] = strconv.FormatBool(settings.ForceEmailOnOIDCAccountCreation)
	return updates, nil
}
...
// GetAccountCreationEmailSuffixWhitelist returns the normalized account creation email suffix whitelist.
func (s *SettingService) GetAccountCreationEmailSuffixWhitelist(ctx context.Context) []string {
	value, err := s.settingRepo.GetValue(ctx, SettingKeyAccountCreationEmailSuffixWhitelist)
	if err != nil {
		return []string{}
	}
	return ParseAccountCreationEmailSuffixWhitelist(value)
}
...
func (s *SettingService) InitializeDefaultSettings(ctx context.Context) error {
	...
	defaults := map[string]string{
		...
		SettingKeyForceEmailOnOIDCAccountCreation:        "false",
...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	result := &SystemSettings{
		EmailVerifyEnabled:                  emailVerifyEnabled,
		AccountCreationEmailSuffixWhitelist: ParseAccountCreationEmailSuffixWhitelist(settings[SettingKeyAccountCreationEmailSuffixWhitelist]),
		PasswordResetEnabled:                emailVerifyEnabled && settings[SettingKeyPasswordResetEnabled] == "true",
