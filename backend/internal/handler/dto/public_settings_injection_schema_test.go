...
func TestPublicSettingsInjectionPayload_SchemaDoesNotDrift(t *testing.T) {
	...
	dtoOnlyFields := map[string]string{
		...
		// force_email_on_oidc_account_creation lives on the DTO but is not injected via SSR.
		"force_email_on_oidc_account_creation": "auth-source default, not a feature flag",
	}
