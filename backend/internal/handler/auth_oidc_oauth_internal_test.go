...
func TestOIDCOAuthStartSetsOnlyOIDCFlowCookies(t *testing.T) {
	handler := &AuthHandler{
		cfg: &config.Config{
	...
	req := httptest.NewRequest(http.MethodGet, "/api/v1/auth/oauth/oidc/start?intent=bind_current_user&redirect=/profile", nil)
	c.Request = req
	...
	require.Contains(t, recorder.Header().Get("Location"), "issuer.example.com/oauth/authorize")
