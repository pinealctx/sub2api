...
func TestGatewayPolicyErrorDetails_MapsGroupRPMExceededToTooManyRequests(t *testing.T) {
	status, code, msg, retryAfter := gatewayPolicyErrorDetails(service.ErrGroupRPMExceeded)
...
func TestGatewayPolicyErrorDetails_MapsUserRPMExceededToTooManyRequests(t *testing.T) {
	status, code, msg, retryAfter := gatewayPolicyErrorDetails(service.ErrUserRPMExceeded)
	require.Equal(t, http.StatusTooManyRequests, status)
	require.Equal(t, "rate_limit_exceeded", code)
	require.NotEmpty(t, msg)
	require.Greater(t, retryAfter, 0, "RPM exceeded should return positive Retry-After")
...
func TestGatewayPolicyErrorDetails_APIKeyRateLimitStillMaps(t *testing.T) {
	...
	for _, err := range []error{
		...
		status, code, _, _ := gatewayPolicyErrorDetails(err)
		require.Equal(t, http.StatusTooManyRequests, status, "status for %v", err)
		require.Equal(t, "rate_limit_exceeded", code)
	}
}

func TestGatewayPolicyErrorDetails_QuotaPolicyServiceUnavailableMapsTo503(t *testing.T) {
	status, code, _, retryAfter := gatewayPolicyErrorDetails(service.ErrQuotaPolicyServiceUnavailable)
	require.Equal(t, http.StatusServiceUnavailable, status)
	require.Equal(t, "gateway_policy_error", code)
	require.Equal(t, 0, retryAfter, "non-RPM errors should not set Retry-After")
}

func TestGatewayPolicyErrorDetails_UnknownErrorFallsBackTo403(t *testing.T) {
	status, code, msg, _ := gatewayPolicyErrorDetails(errors.New("unknown gateway policy error"))
...
func TestGatewayPolicyErrorDetails_T10_QuotaExhaustedReturns429WithRetryAfter(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			status, code, _, retryAfter := gatewayPolicyErrorDetails(tc.err)
